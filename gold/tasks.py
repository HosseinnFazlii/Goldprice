...
def generate_telegram_message(prices):
    ...
    for original_title, price in prices.items():
        # Normalize title (strip spaces and parentheses)
        normalized_title = original_title.replace(" ", "").replace("(", "").replace(")", "").strip()
        telegram_title = title_mapping.get(normalized_title, original_title)  # Use mapped title
